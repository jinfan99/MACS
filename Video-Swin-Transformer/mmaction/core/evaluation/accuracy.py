...
def confusion_matrix(y_pred, y_real, normalize='all'):
...
def cal_confusion_mat(y_pred, y_real):
    y_pred = np.array(y_pred)
    y_real = np.array(y_real)

    TP = np.where((y_pred == y_real) & (y_pred == 1), 1, 0).sum()
    TN = np.where((y_pred == y_real) & (y_pred == 0), 1, 0).sum()
    FP = np.where((y_pred != y_real) & (y_pred == 1), 1, 0).sum()
    FN = np.where((y_pred != y_real) & (y_pred == 0), 1, 0).sum()
